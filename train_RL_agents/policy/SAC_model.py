...
class Actor(nn.Module):
    def __init__(self,
        ...
        self.min_val = torch.tensor(1e-6).float().to(self.device)
        ...
        # output layers
        self.output_layer_mu = nn.Linear(hidden_dimension, self.action_dimension)
        self.output_layer_log_std = nn.Linear(hidden_dimension, self.action_dimension)
    ...
    def forward(self, x):
        ...
        return self.compute_action(mu, log_std)
    
    def compute_action(self, mu, log_std):
        sigma = torch.exp(log_std).to(self.device)
        dist = Normal(mu,sigma)
        z = dist.rsample().to(self.device)
        
        # map to (-1,1)
        actions = self.atan_scale*torch.atan(z)
        log_prob = dist.log_prob(z) - torch.log(1 - actions.pow(2) + self.min_val)
