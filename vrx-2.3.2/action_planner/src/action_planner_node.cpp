...
// class VO_agent : public ActionPlannerNode
// {
// public:
//     VO_agent(const std::string& robot_name) : ActionPlannerNode(robot_name) {
//         m = 400.0;
//         Izz = 450.0;
//         width = 2.5;
//         length = 5.0;
...
//         planned_vel = Eigen::Vector2f::Zero();
...
//         else virtual_objs.push_back(obj_pos);
//     }
//     void computeVel(const state_msg::msg::State::SharedPtr msg){
        
//         Eigen::Vector2f goal(msg->goal.x,msg->goal.y);
//         // Desired Velocity (towards goal)
//         Eigen::Vector2f v_desire = 3.0 * goal / goal.norm();

//         Eigen::Vector2f vA(msg->self_velocity.linear.x,msg->self_velocity.linear.y);
//         Eigen::Vector2f pA(0.0,0.0);

//         std::vector<std::vector<Eigen::Vector2f>> VO_BA_all_vec;
//         std::vector<std::vector<float>> VO_BA_all_f;

//         for(int i=0; i<msg->object_positions.size(); i++){
//             Eigen::Vector2f pB(msg->object_positions[i].x,msg->object_positions[i].y);
//             Eigen::Vector2f vB(msg->object_velocities[i].x,msg->object_velocities[i].y);

//             if(vB.norm()>=0.5){
//                 // Viewed as dynamic objects
                
//                 // check COLREGs
//                 std::vector<Eigen::Vector2f> virtual_objs;
//                 check_COLREGs(vA,pB,vB,virtual_objs);

//                 for(auto obj:virtual_objs){
//                     Eigen::Vector2f transl_vB_vA(pA(0)+0.5*(vB(0)+vA(0)),pA(1)+0.5*(vB(1)+vA(1)));
                
//                     float dist_BA = distance(pA,obj);
//                     float theta_BA = std::atan2(obj(1)-pA(1),obj(0)-pA(0));

//                     float min_dist = 2 * (msg->self_radius + msg->object_radii[i]);
//                     if(min_dist > dist_BA) dist_BA = min_dist;

//                     float theta_BAort = std::asin(min_dist/dist_BA);
//                     float theta_ort_left = theta_BA+theta_BAort;
//                     Eigen::Vector2f bound_left(std::cos(theta_ort_left), std::sin(theta_ort_left));
//                     float theta_ort_right = theta_BA-theta_BAort;
//                     Eigen::Vector2f bound_right(std::cos(theta_ort_right), std::sin(theta_ort_right));

//                     VO_BA_all_vec.push_back({transl_vB_vA,bound_left,bound_right});
//                     VO_BA_all_f.push_back({dist_BA,min_dist});
//                 }
//             }
//             else{
//                 vB = Eigen::Vector2f::Zero();

//                 Eigen::Vector2f transl_vB_vA(pA(0)+vB(0),pA(1)+vB(1));
                
//                 float dist_BA = distance(pA,pB);
//                 float theta_BA = std::atan2(pB(1)-pA(1),pB(0)-pA(0));

//                 float OVER_APPROX_C2S = 1.5;
//                 float rad = 2 * (msg->object_radii[i]) * OVER_APPROX_C2S;
//                 float min_dist = rad + msg->self_radius;
//                 if(min_dist > dist_BA) dist_BA = min_dist;
...
//         planned_vel = intersect(pA, v_desire, VO_BA_all_vec, VO_BA_all_f);
//     }
//     void computeAction(const state_msg::msg::State::SharedPtr msg){
//         update_model_parameters(msg);
...
//             // Compute the projection of the new velocity in the direction of planned velocity
//             float proj = planned_vel.dot(V_r_next_2d)/planned_vel.norm();

//             // Compute the difference in angle between the next heading and the direction of planned velocity
//             float F_angle = std::atan2(planned_vel(1),planned_vel(0));
//             float angle_diff = std::abs(warpToPi(heading_next-F_angle));
...
//         b_V = -1.0 * C_RB * V - N * V_r;
//     }
//     Eigen::Vector2f intersect(const Eigen::Vector2f& pA, const Eigen::Vector2f& vA, 
//                               const std::vector<std::vector<Eigen::Vector2f>>& VO_BA_all_vec,
//                               const std::vector<std::vector<float>>& VO_BA_all_f){
//         Eigen::Vector2f zeros(0.0,0.0);
//         float norm_v = distance(vA, zeros);
//         std::vector<Eigen::Vector2f> suitable_V;
//         std::vector<Eigen::Vector2f> unsuitable_V;
//         for(int i=0; i<10; i++){
//             float theta = i * M_PI / 5;
//             for(int j=0; j<50; j++){
//                 float rad = 0.02 + i * norm_v / 50;
//                 Eigen::Vector2f new_v(rad * std::cos(theta), rad * std::sin(theta));
//                 bool suit = true;
//                 for(int k=0; k<VO_BA_all_vec.size();k++){
//                     auto& p_0 = VO_BA_all_vec[k][0];
//                     auto& left = VO_BA_all_vec[k][1];
//                     auto& right = VO_BA_all_vec[k][2];
//                     Eigen::Vector2f dif(new_v(0)+pA(0)-p_0(0), new_v(1)+pA(1)-p_0(1));
//                     float theta_dif = std::atan2(dif(1),dif(0));
//                     float theta_right = std::atan2(right(1),right(0));
//                     float theta_left = std::atan2(left(1),left(0));
//                     if(in_between(theta_right,theta_dif,theta_left)){
//                         suit = false;
//                         break;
//                     }
//                 }
//                 if(suit) suitable_V.push_back(new_v);
//                 else unsuitable_V.push_back(new_v);
//             }
//         }

//         Eigen::Vector2f new_v(vA);
//         bool suit = true;
//         for(int k=0; k<VO_BA_all_vec.size();k++){
//             auto& p_0 = VO_BA_all_vec[k][0];
//             auto& left = VO_BA_all_vec[k][1];
...
//         Eigen::Vector2f vA_post(0.0,0.0);
//         if(!suitable_V.empty()){
//             float min_score = std::numeric_limits<float>::infinity();
//             for(auto vel:suitable_V){
//                 float score = distance(vel,vA);
//                 if(score < min_score){
//                     min_score = score;
//                     vA_post = vel;
//                 }
...
//         }
//         else{
//             std::vector<float> tc_V;
//             for(auto unsuit_v:unsuitable_V){
//                 float min_tc = std::numeric_limits<float>::infinity();
//                 for(int k=0; k<VO_BA_all_vec.size();k++){
//                     auto& p_0 = VO_BA_all_vec[k][0];
//                     auto& left = VO_BA_all_vec[k][1];
//                     auto& right = VO_BA_all_vec[k][2];
//                     auto& dist = VO_BA_all_f[k][0];
//                     float rad = VO_BA_all_f[k][1];
//                     Eigen::Vector2f dif(unsuit_v(0)+pA(0)-p_0(0),unsuit_v(1)+pA(1)-p_0(1));
//                     float theta_dif = std::atan2(dif(1),dif(0));
//                     float theta_right = std::atan2(right(1),right(0));
//                     float theta_left = std::atan2(left(1),left(0));
//                     if(in_between(theta_right,theta_dif,theta_left)){
//                         float small_theta = std::abs(theta_dif-0.5*(theta_left+theta_right));
//                         if(std::abs(dist * std::sin(small_theta)) >= rad){
//                             rad = std::abs(dist * std::sin(small_theta));
//                         }
//                         float big_theta = std::asin(std::abs(dist*std::sin(small_theta))/rad);
//                         float dist_tg = std::abs(dist*std::cos(small_theta))-std::abs(rad*std::cos(big_theta));
//                         if(dist_tg < 0.0) dist_tg = 0.0;
//                         Eigen::Vector2f zeros(0.0,0.0);
//                         float tc_v = dist_tg/distance(dif, zeros);
//                         if(tc_v < min_tc) min_tc = tc_v;
//                     }
//                 }
//                 tc_V.push_back(min_tc+0.001);
//             }

//             float min_score;
//             for(int i=0; i<unsuitable_V.size(); i++){
//                 float score = 0.2/tc_V[i] + distance(unsuitable_V[i],vA);
//                 if(i==0){
//                     min_score = score;
//                     vA_post = unsuitable_V[i];
//                 }
//                 else{
//                     if(score < min_score){
//                         min_score = score;
...
//                 theta_left += 2 * M_PI;
//                 if(theta_dif<0) theta_dif += 2 * M_PI;
//                 if((theta_right <= theta_dif) && (theta_dif <= theta_left)){
//                     return true;
//                 }
//                 else return false;
...
//     float distance(const Eigen::Vector2f& v1, const Eigen::Vector2f& v2){
//         return (v1-v2).norm();
//     }
...
//     Eigen::Vector2f planned_vel; // VO planned velocity
